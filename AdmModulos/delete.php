...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($idM)) {
        echo json_encode(["status" => "error", "message" => "ID de módulo vacío."]);
        exit();
    }

    // Consulta de eliminación
    $stmt = $conn->prepare("DELETE FROM Modulo WHERE idM = ?");
    $stmt->bind_param("i", $idM);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Módulo eliminado correctamente."]);

        // Registrar actividad de eliminación
        $modulo = "Gestión de Módulos";
        $descripcion = "Eliminación del módulo con ID: $idM";
        $estado = "Completado";
        registrar_actividad($idU, $modulo, $descripcion, $estado, $conn);
    } else {
        echo json_encode(["status" => "error", "message" => "Error al eliminar el módulo: " . $stmt->error]);
    }
